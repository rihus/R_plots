...
#Load data file (CFBronc_boxplot_N4corr_data.csv): Only N4 corrected plot (VDP & Hyper)
df_4bxp_N4 <- read.csv(file.choose())
compare_means(VDP_percent ~ Sequence, data = df_4bxp_N4, method = "wilcox.test", paired = TRUE)
p <- ggboxplot(df_4bxp_N4, x = "Sequence", y = "VDP_percent",
               shape = 19,size=1, ylim = c(0, 25),
               ...
               stat_compare_means(method = "wilcox.test", paired = TRUE,label.y = 24,
...
compare_means(HVP_percent ~ Sequence, data = df_4bxp_N4, method = "wilcox.test", paired = TRUE)
p <- ggboxplot(df_4bxp_N4, x = "Sequence", y = "HVP_percent",
               shape = 19,size=1, ylim = c(0, 0.75),
               ylab = "HVP (%)", xlab = "Sequence",
               fill = "Sequence", palette = "cincy_col", add = "jitter",
               add.params = list(color = "black", size=3),
               bxp.errorbar = TRUE,bxp.errorbar.width = 0.5, legend = "none",
               font.x = c(22, "bold", "black"),
               ...
               font.tickslab = c(22, "bold", "black"),
               caption="Wilcoxon signed rank test") +
  stat_compare_means(method = "wilcox.test", paired = TRUE,label.y = 0.7,
                     aes(label = paste0("P = ", ..p.format..)), size=8) +
  font("caption", size = 12, color = "gray", face = "bold.italic")
