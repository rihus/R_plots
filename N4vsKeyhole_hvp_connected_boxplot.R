...
stat.test <- vdp60_combined %>%
  wilcox_test(HyperP ~ Correction , paired = TRUE) %>%
  add_significance()
stat.test
# Box plots with p-values HVP
p_bxp60 <-  ggpaired(vdp60_combined, x = "Correction", y = "HyperP", fill = "Correction",
                     palette = c("#095859", "#DB9C60"), width = 0.5, point.size = 2, point.color="Subject",
                     ylim = c(0, 8), line.color = "gray", line.size = 0.5,
                     legend = "none", ylab = "HVP (%)", xlab = "Correction Method") + # 
...
p_bxp60 <- p_bxp60 + stat_pvalue_manual(stat.test, label = "p.signif", y.position = 7.5,
...
ggsave("Rplot_N4vsFA_hvp60_bxp_connected.png", plot = p_bxp60, width = 4.25, height = 3.35, dpi = 300)
...
stat.test
# Box plots with p-values HVP
p_bxp_prcntl <-  ggpaired(percentile_combined, x = "Correction", y = "HyperP", fill = "Correction",
                          palette = c("#095859", "#DB9C60"), width = 0.5, point.size = 2, point.color="Subject",
                          ylim = c(0, 8), line.color = "gray", line.size = 0.5,
                          legend = "none", ylab = "HVP (%)", xlab = "Correction Method") + #
  theme(panel.border = element_rect(color = "black", fill = NA, linewidth = 1),
        ...
        axis.title = element_text(size = 22, color = "black", face = "bold"))
stat.test <- stat.test %>% add_xy_position(x = "Correction")
p_bxp_prcntl <- p_bxp_prcntl + stat_pvalue_manual(stat.test, label = "p.signif", y.position = 7.5,
                                                  label.size = 8, bracket.size = 0.8, tip.length = 0.02)
##axis.line.x = element_line(linewidth = 1), axis.line.y = element_line(linewidth = 1)
print(p_bxp_prcntl)
# Save the plot as a png file in the current directory
ggsave("Rplot_N4vsFA_hvpprcntl_bxp_connected.png", plot = p_bxp_prcntl, width = 4.25, height = 3.35, dpi = 300)
#########################################
...
# Box plots with p-values HVP
p_bxp_median <-  ggpaired(median_combined, x = "Correction", y = "HyperP", fill = "Correction",
                          palette = c("#095859", "#DB9C60"), width = 0.5, point.size = 2, point.color="Subject",
                          ylim = c(0, 22), line.color = "gray", line.size = 0.5,
                          legend = "none", ylab = "HVP (%)", xlab = "Correction Method") + #
  theme(panel.border = element_rect(color = "black", fill = NA, linewidth = 1),
        axis.text = element_text(size = 22, color = "black", face = "bold"),
        axis.title = element_text(size = 22, color = "black", face = "bold"))
stat.test <- stat.test %>% add_xy_position(x = "Correction")
p_bxp_median <- p_bxp_median + stat_pvalue_manual(stat.test, label = "p.signif", y.position = 20.5,
                                                  label.size = 8, bracket.size = 0.8, tip.length = 0.02)
##axis.line.x = element_line(linewidth = 1), axis.line.y = element_line(linewidth = 1)
print(p_bxp_median)
# Save the plot as a png file in the current directory
ggsave("Rplot_N4vsFA_hvpmedian_bxp_connected.png", plot = p_bxp_median, width = 4.25, height = 3.35, dpi = 300)
